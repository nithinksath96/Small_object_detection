...
def init_detector(opts, device='cuda:0'):
    ...
    if 'center_crop' in opts and opts.center_crop:
        config.data.test.center_crop = {'y': 500, 'h': 256}
    else:
        config.data.test.center_crop = None
...
def _prepare_data(img, img_transform, cfg, device):
    cc_cfg = cfg.data.test.center_crop
    if cc_cfg is not None:      
        img = img[cc_cfg['y']: cc_cfg['y'] + cc_cfg['h']]
    ori_shape = img.shape
...
def inference_detector(model, img, gpu_pre=True, numpy_res=True, decode_mask=True):
    ...
    with torch.no_grad():
        ...
        cc_cfg = cfg.data.test.center_crop
        if cc_cfg is not None and len(result[0]):
            result[0][:, [1, 3]] += cc_cfg['y']
    return result
